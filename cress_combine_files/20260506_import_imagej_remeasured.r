...
#     20260506_build_filename_lookup.r).
...
if (is.null(lookup_file)) {
  stop("No filename_to_bag_lookup.xlsx found in any *_cress_lookup/ folder. ",
       "Run 20260506_build_filename_lookup.r first.")
}
cat("Using lookup:", lookup_file, "\n")
cat("Output folder:", output_dir, "\n\n")

# The remeasurement xlsx lives one directory up from this script's working
# folder (project root). Sheet "A" is the only sheet. Row 1 holds image
# filenames in columns 1, 5, 9, ...; row 2 holds the repeated subheader
...
input_file <- "../260506_ASPS1-5_cress_measures.xlsx"
...
# Match the exact column order used by 20251022_cress_combine_files.r so the
