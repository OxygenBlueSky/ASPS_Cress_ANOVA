...
# Notes:
# - ASPS2 ungerade has 11-B-8 !!! check for copy
# - trace missing bags
...
compute_emmeans_contrasts <- function(data_subset, variable) {
  ...
  # Below is misleading, as its type I SS for pairwise comparisons, 
  # and showing it here just shows that it is not the same 
  #as the general Type III used aboev
...
cat("\n\n")
cat(strrep("#", 80), "\n")
cat("CREATING NORMALIZED PLOTS\n")
cat(strrep("#", 80), "\n")
cat("Normalizing all values to Lactose grand mean\n")
cat(strrep("#", 80), "\n\n")

# Create normalized datasets and plots for each variable
plot_files <- list()
...
for (var in response_vars) {
  ...
  # Calculate Lactose grand mean across all experiments
  lactose_grand_mean <- df_bags %>%
    filter(potency == "Lactose") %>%
    pull(!!sym(var)) %>%
    mean(na.rm = TRUE)
  
  cat(sprintf("  Lactose grand mean: %.3f\n", lactose_grand_mean))
  
  # Normalize all values by dividing by Lactose grand mean
  df_normalized <- df_bags %>%
    mutate(
      normalized_value = !!sym(var) / lactose_grand_mean
    )
  
  # Create list to store individual experiment plots
  exp_plots <- list()
  ...
  for (exp in experiments) {
    ...
    # Get individual bag data for this experiment (NOT summary stats)
    exp_data <- df_normalized %>%
      filter(experiment_number == exp)
  ...
  # Arrange all 5 experiment plots horizontally with shared legend below
  ...
  # Save plot
  output_plot <- paste0(date2, "_cress_normalized_", var, ".png")
  ggsave(
  ...
  cat(sprintf("  Saved: %s\n", output_plot))
  plot_files[[var]] <- output_plot
}

cat("\n")
cat(strrep("=", 80), "\n")
cat("All normalized plots created (by experiment)\n")
cat(strrep("=", 80), "\n\n")
...
cat(strrep("#", 80), "\n")
cat("Showing each potency across experiments\n")
cat(strrep("#", 80), "\n\n")

# Create normalized datasets and plots for each variable (by potency view)
plot_files_potency <- list()
...
for (var in response_vars) {
  ...
  # Create list to store individual potency plots
  potency_plots <- list()
  
  # Get unique potencies in order
  potencies <- sort(unique(df_normalized$potency))
  
  # Create a boxplot for each potency
  for (pot in potencies) {
    
    # Get individual bag data for this potency
    pot_data <- df_normalized %>%
      filter(potency == pot)
  ...
  # Arrange all 6 potency plots horizontally with shared legend below
  combined_plot <- gridExtra::grid.arrange(
    grobs = potency_plots,
    ncol = 6,
    nrow = 1,
    top = grid::textGrob(
      paste0(var, " by Potency\nNormalized to Lactose grand mean = ", 
             sprintf("%.3f", lactose_grand_mean)),
      gp = grid::gpar(fontsize = 14, fontface = "bold")
    ),
    bottom = legend
  ...
  # Save plot
  output_plot <- paste0(date2, "_cress_normalized_by_potency_", var, ".png")
  ggsave(
    filename = output_plot,
    plot = combined_plot,
    width = 42,
  ...
  cat(sprintf("  Saved: %s\n", output_plot))
  plot_files_potency[[var]] <- output_plot
}

cat("\n")
cat(strrep("=", 80), "\n")
cat("All normalized plots created (by potency)\n")
cat(strrep("=", 80), "\n\n")
