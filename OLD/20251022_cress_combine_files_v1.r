...
# Updated: 2025-10-22 14:30
# 
# Changes in this version:
# - ASPS 6-10: Use LASPR1/2 and LAGES1/2 columns (correctly halved values)
# - ASPS 6-10: Calculate root_length = seedling_length - sprout_length
# - Added removal of doublets (entries with blank potency) at end
# - Updated code style to avoid delimiter lines

# "Count" is just sequential numbering for ASPS 1-5, 
# as this is not recorded for those data processed by Paul (oct 13.-20. 2025)

# This version uses LASPR 1/2 as the correct values for ASPS 6-10
# Also removes all copies that were in the data because of photo dublets

# 🤪 Ars. album and Ars.Album issue persist

# -----!! Just uses files that are in same folder !!
...
read_asps_6_10 <- function() {
  ...
  # Use the halved columns (LASPR1/2 and LAGES1/2) as the correct values
  # The original sprout_length and seedling_length were outputted incorrectly (2x too large)
  df <- df %>%
    mutate(
      sprout_length = `LASPR1/2`,
      seedling_length = `LAGES1/2`,
      root_length = seedling_length - sprout_length  # Recalculate root as seedling minus sprout
    ) %>%
    select(reference_cell, count, label, sprout_length, seedling_length,
           root_length, root_sprout_ratio, exp_no, bag, potency)
...
main <- function() {
  ...
  # Check for any missing potencies before filtering
  missing_potencies <- sum(is.na(combined_data$potency) | combined_data$potency == "")
  if (missing_potencies > 0) {
    cat("Found", missing_potencies, "rows with missing potency values (will be removed as doublets)\n")
  }
  
  
  # Removal of doublets
  # Filter out entries where potency is blank or NA
  
  combined_data <- combined_data %>%
    filter(!is.na(potency) & potency != "")
  
  cat("After removing doublets:", nrow(combined_data), "rows remaining\n\n")
...
# 5. ASPS 6-10: Using LASPR1/2 and LAGES1/2 columns as these are the correctly halved values
#    The original sprout_length and seedling_length were outputted at 2x the correct size
# 6. ASPS 6-10: root_length is recalculated as seedling_length - sprout_length
# 7. Doublets (entries with blank potency) are removed at the end of processing